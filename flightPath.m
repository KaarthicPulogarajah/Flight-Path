...
%     f2 = figure('Name', '3D Sine Curve', 'NumberTitle', 'off');
    ...
    thrust = 97900*2;
    mass = input('\nEnter mass(kg) of Boeing 737-300 to generate takeoff flight path\nMin: 33000 kg\nMax: 63000 kg\n');
    ...
    clearXY = vpa(solve(takeoffFunc == 10.7, t),5);
...
%         fprintf('no');
    ...
    %function to model angle of ascent increasing by 2 deg/s
%     takeoffX = linspace(-6.5*Vr,0,100);
%     takeoffY = takeoffX;
%     takeoffZ = -180/(2*pi)*Vr*log(cos(2*pi/(180*Vr)*(takeoffX+6.5*Vr)));
%     
%     takeoffCurve = [takeoffX;takeoffY;takeoffZ];
% 
%     syms x;
%     clearXY = vpa(solve( -180/(2*pi)*Vr*log(cos(2*pi/(180*Vr)*(x+6.5*Vr))) == 10.7, x),5);
    ...
    %GIVES THAT 10.7m IS CLEARED AT 3.84s, CONSISTENT WITH "target liftoff
    ...
    legend = {'Point A','Closest Point','Aircraft Initial Position','Vr Point','Take Off Point','Clear Point (35 ft)','Runway', 'Flight Vector', 'Flight Curve'};

    width = 800;
    f1 = figure('Name','example1','NumberTitle', 'off', 'Position', [100,100,width*4/3,width]);
...
function plot3d(points,vectors,curves,names)
    ...
    axis([-2700 1500 -2700 1500 0 500]);
    xlabel('X');
    ylabel('Y');
    zlabel('Z');
    box on;
...
shortestDist = sqrt(pointVectorMag^2 - flightVectorProj^2);
